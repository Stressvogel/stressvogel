...
# List of library_old directories for -L linker option (-L added when used).
# Includes the BSP.
ALT_LIBRARY_DIRS :=

# List of library_old names for -l linker option (-l added when used).
# Includes the BSP.
ALT_LIBRARY_NAMES :=

# List of library_old names for -msys-lib linker option (-msys-lib added when used).
# These are libraries that might be located in the BSP and depend on the BSP
# library_old, or vice versa
ALT_BSP_DEP_LIBRARY_NAMES :=

# List of dependencies for the linker.  This is usually the full pathname
# of each library_old (*.a) file.
# Includes the BSP.
ALT_LDDEPS :=

# List of root library_old directories that support running make to build them.
...
# 
ELF := MediaComputerVGAPoC.elf

# Paths to C, C++, and assembly source files.
C_SRCS :=
CXX_SRCS += main.cpp
CXX_SRCS += library/vga/vga.cpp
ASM_SRCS :=
...
# Detect if small newlib C library_old is requested.
...
# Put each BSP dependent library_old in a group to avoid circular dependencies.
